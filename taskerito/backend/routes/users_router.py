...
from dependencies import get_db, verify_admin, verify_manager, get_current_user
...
def get_all_users(current_user: User = Depends(verify_manager), db: Session = Depends(get_db)):
    service = UserService(db)
